...
class BasePrior(nn.Module, metaclass=ABCMeta):
    ...
    Derive this class to make custom prior models,
     which take some arguments and a dataset as input.
    ...
    def get_init_args(self):
        ...
        The values should be returned inside a dict
         with the keys being the arguments' names.
        All values should also be saveable in a .yaml file
        as this is used to reconstruct the
...
class Atomref(BasePrior):
    ...
    def __init__(self, max_z=None, dataset=None):
        ...
        if dataset is None:
            ...
            if atomref is None:
                rank_zero_warn(
                    "The atomref returned by the dataset is None,"
                    " defaulting to zeros with max. "
                    "atomic number 99. Maybe atomref is not defined"
