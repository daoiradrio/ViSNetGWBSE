...
class QM9GWBSE(torch.nn.Module):
    ...
    def _read_data(self, data_path, target, num_train, num_val, num_test):
        ...
        self.train_dataset = QM9GWBSEDataset(num_train, N_train, Z_train, R_train, M_train, E_train)
        self.val_dataset = QM9GWBSEDataset(num_val, N_val, Z_val, R_val, M_val, E_val)
        self.test_dataset = QM9GWBSEDataset(num_test, N_test, Z_test, R_test, M_test, E_test)
    ...
    def _prepare_data(self, data_path, target, num_train, num_val, num_test, remove_charged):
        ...
        for split_set, split_idx in split_sets.items():
            ...
            os.makedirs(os.path.join(data_path, f"{split_set}_data"))
            for i in tqdm(split_idx, leave=False):
                ...
                elif target == "DELTABSE":
                    edft_homo = torch.tensor([np.loadtxt(f"{dft_path}/{mol}.dat")[homo_idx]]) * HARTREE_TO_EV
                    edft_lumo = torch.tensor([np.loadtxt(f"{dft_path}/{mol}.dat")[homo_idx+1]]) * HARTREE_TO_EV
                    ebse = torch.tensor([np.loadtxt(f"{exc_ss_path}/{mol}.dat")[0] * HARTREE_TO_EV])
                    E = ebse - (edft_lumo - edft_homo)
                all_N.append(N)
                all_Z.append(Z)
                all_R.append(R)
                all_M.append(M)
